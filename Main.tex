...
\lstdefinelanguage{Kotlin}{
  ...
  keywords={abstract, actual, as, as?, break, by, class, companion, continue, data, do, dynamic, else, enum, expect, false, final, for, fun, get, if, import, in, interface, internal, is, null, object, override, package, private, cpublic, return, set, super, suspend, this, throw, true, try, typealias, val, var, vararg, when, where, while},
...
\begin{document}
	...
	\chapter*{Úvod}
		...
		\gls{Kotlin} je ideální programovací jazyk pro vývoj knihovny, protože je interoperabilní s~Javou, Javascriptem i~C, a~tak umožňuje tuto knihovnu používat jak pro \gls{JVM}, tak i~v~prohlížeči nebo v~programech kompilovaných přímo do binárního kódu.
	...
	\part{Teoretická část}
			...
			\chapter{Laický náhled na neuronové sítě}
				...
				\section{Sítě} \label{s:net}
					Jelikož \uv{nahodilé neurony} by se těžko udržovaly v~paměti a~operace na nich by byly velmi pomalé, potřebujeme síť nějak uspořádat. Nejjednodušším uspořádáním jsou \emph{vrstvy}. Každý neuron z~jedné vrstvy má \gls{dendrit}y ze všech neuronů z~vrstvy minulé. Tak se předejde \glsdisp{cyklus}{cyklům}, které jsou složité na výpočty, a~navíc si nemusíme u~každého neuronu pamatovat, ze kterých neuronů do něj vede signál.
					...
					\begin{figure}[h]
						...
						\caption{Běžná neuronová síť ($\W$ jsou váhy, $n$ neurony a $v$ je výstupní signál, viz kapitola \ref{s:fn} a sekce \ref{s:n})}
					\end{figure}
					...
					\begin{figure}[h]
						\resizebox{\textwidth}{!}{%
							\begin{tikzpicture}
								...
								\node (S) at ($(NI)!.65!(N)$) {Synapse z $n_{i*}$ do $n$};
								\node at ($(S)!.48!(N)$) {Dendrity};
								\node at ($(NI)!.5!(S)$) {Axony vstupních neuronů};
					    		...
					    		\foreach \x in {0,1,...,\first}{%
						    		\draw [->] (A\x) --node[fill=white, align=center] {\scriptsize $v_{i\x}$\\[-1mm] $\rightarrow$} ($(A\x)!.2!(B)$) -- ($(A\x)!.4!(B)$) -- node[fill=white] {\scriptsize $\W(n_{i\x}, n)$} (B);
							    }
     
				    			\foreach \y in {0,1,...,\third}{%
							    	\draw [->] (B)  -- node[fill=white, align=center] {\scriptsize $v$\\[-1mm] $\rightarrow$} ($(B)!.7!(C\y)$) -- (C\y);
		    					}
						    \end{tikzpicture}
						}
						\caption{Neuron}
