...
\begin{document}
	...
	\chapter*{Úvod}
		...
		V~textu jsou použita pojmy ze stavby biologického neuronu, objektově orientovaného programování, Kotlinu, \ldots Tyto pojmy jsou vysvětleny na konci práce. 
	...
	\part{Teoretická část}
			...
			\chapter{Laický náhled na neuronové sítě}
				...
				\section{Neuron}
					...
					Podrobněji o souvislosti biologických a umělých neuronových sítích pojednává \autocite{book:BNN}.
						...
						\resizebox{\textwidth}{!}{%
							\begin{tikzpicture}
								...
								\foreach \x in {0,1,...,\first}{%
							    	...
							    	\node[align=center] at ($(A\x)+(0.7,0.7)$) {$v_{\x,\,1}$\\[-1mm] $\rightarrow$};
							    }
     
								\foreach \x in {0,1,...,\second}{%
								...
								\foreach \x in {0,1,...,\fourth}{%
							    	\node[X] (D\x) at (25, 2*\x-\fourth) {$n_{\x,\,4}$};
							    }
     
					    		\foreach \x in {0,1,...,\first}{%
					    			\foreach \y in {0,1,...,\second}{%
			...
			\chapter{Formální náhled} \label{s:fn}
				...
				\section{Definice neuronu a sítě}
				...
				\section{Chybová funkce}
					...
					V knihovně je implementována (a to přímo \uv{natvrdo} ve třídě \verb!BasicNeuralNetwork!) pouze jediná chybová funkce
				...
				\section{Zpětná propagace}
					Při zpětné propagaci je důležitý vzorec pro derivaci složené funkce, někdy také znám jako \uv{řetízkové pravidlo} (pro funkci jedné proměnné platí (\ref{eq:rp1}), pro více pak (\ref{eq:rp2}))\footnote{Pro funkce musí platit, že mají v daných bodech derivaci, viz \autocite[s. 623]{book:Matanalysis}.}
					...
					Vektor $\frac{\delta E}{\delta\vec{w}}$ už stačí jen vynásobit \verb!learningRate! a můžeme ho přičíst k $\vec{w}$, abychom upravili hodnoty $W\left(n, n_x\right)$.
				...
				\section{Shrnutí}
					Cílem je tedy implementovat rovnice:
					\begin{itemize}
				...
				\section{Aktivační funkce} \label{s:af}
					...
					\begin{itemize}
						\item \emph{Binary step}
							...
							\begin{figure}[h]
						        \centering
					        	\includegraphics[width=0.3\textwidth]{graphs/binaryStep}
						...
						\item \emph{Identity}
						    ...
						    \begin{figure}[h]
						    	\centering
					    	    \includegraphics[width=0.3\textwidth]{graphs/identity}
					        	\caption{Identita}
						    \end{figure}
						...
						\item \emph{Sigmoid} \autocite{article:AF} (značí se $\sigma$)
							...
							je jedna z~nejznámějších aktivačních funkcí. Je to vlastně takový hladký přechod mezi 0 a~1. Také je na $\sigma$ dobře vidět, proč se často počítá derivace z~funkční hodnoty, místo počítání exponenciální funkce a~dělení si vystačíme s~násobením a~odčítáním.
					   		...
					   		\begin{figure}[h]
						        \centering
					        	\includegraphics[width=0.3\textwidth]{graphs/sigmoid}
						        \caption{$\sigma$}
						    \end{figure}
						...
						\item Nesmíme zapomenout na \emph{sigmoidě} podobnou a~také často používanou funkci \emph{hyperbolický tangens} ($\tanh$) \autocite{article:AF} \autocite{book:FFActivationFunctions}:
				    		...
				    		\begin{figure}[h]
					        	\centering
					        	\includegraphics[width=0.3\textwidth]{graphs/tanh}
						...
						\item Další funkce s~vazbou na \emph{sigmoidu} je funkce \emph{swish} \autocite{article:AF}:
					    	...
					    	\begin{figure}[h]
					    		\centering
						        \includegraphics[width=0.3\textwidth]{graphs/swift}
						        \caption{Swish}
						    \end{figure}
						...
						\item Ukazuje se, že identita jako taková se v~podstatě použít nedá, ale hojně využívaná je její \uv{upravená} verze \emph{rectified linear unit} \autocite{article:AF} (česky něco jako \emph{napravená přímá úměrnost}), která záporná čísla převádí na nulu a~v~kladných se chová jako \emph{identita}:
						    ...
						    \begin{figure}[h]
						    	\centering
						        \includegraphics[width=0.3\textwidth]{graphs/rectifiedLinearUnit}
						        \caption{Rectified linear unit}
						    \end{figure}
						
						\item Kromě této verze je v~knihovně ještě \emph{leaky} (děravá či prosakující) \emph{rectified linear unit} \autocite{article:AF}, která v~záporných hodnotách nedává nulu, ale \emph{přímou úměrnost}. K~těmto funkcím můžeme přiřadit i~\emph{hard hyperbolic function}, která je \emph{identitou} pouze na intervalu $(-1, 1)$, tedy odpovídá biologickému neuronu asi nejvíce z~těchto \uv{lineárních funkcí}.
					
						    \begin{figure}[h]
					    		\centering
					        	\includegraphics[width=0.3\textwidth]{graphs/hardHyperbolicFunction}
						...
						\item \emph{Rectified unit} není hladká (nemá derivaci v~bodě nula), ale to lze napravit, když použijeme funkci \emph{soft plus} \autocite{article:AF} ($\ln\left(1+e^x\right)$). Podobnou úpravu lze udělat i~s~funkcí \emph{signum} (\emph{znaménko}, často se značí \emph{sign}), což je téměř \emph{binární krok}\footnote{Z důvodu téhle podobnosti není ani implementována.}, akorát v~záporných hodnotách nabývá funkční hodnoty -1 místo 0. \emph{Signum} se dá zapsat jako podíl $x$ a~$|x|$, tudíž tato úprava (\emph{soft sign} \autocite{article:AF}) vypadá následovně:
							...
							\begin{figure}[h]
							    ...
							    \begin{subfigure}[b]{0.5\textwidth}
				    	    		\includegraphics[width=0.6\textwidth]{graphs/softPlus}
							        \caption{Soft plus}
						    	\end{subfigure}
						...
						\item Jednou skupinou funkcí, se kterými se sice experimentuje, ale stěží najdete nějaké využití, jsou ty, které nejsou monotonní\footnote{Můžeme si všimnout, že téměř všechny předchozí funkce jsou neklesající, většina dokonce rostoucí na celém definičním oboru.}, jako sinus, kosinus, \emph{Gaussova funkce} ($e^{-x^2}$), apod. Vzhledem k~jejich mizivému využití je implementován pouze \emph{sinus}.
							...
							\begin{figure}[h]
								\centering
						        \includegraphics[width=0.3\textwidth]{graphs/sin}
				    	    	\caption{Sinus}
				    		\end{figure}	
					\end{itemize}
	...
	\part{Praktická část}
			...
			Abych nemusel programovat maticové násobení, použil jsem knihovnu \verb!koma! (celým názvem Kotlin math), která implementuje základny lineární algebry v Kotlinu. Knihovna je pro \gls{JVM}, Javascript i pro binární kód, avšak ve Windows ji nelze zkompilovat, proto má knihovna funguje pouze pro \gls{JVM} a Javascript. \autocite{online:Koma}
		...
		\chapter{Struktura knihovny}
			...
			\begin{figure}
				\begin{forest}
					...
					[{NeuralNetwork}
						[{core}, name=p
						  	[{IActivation\\Functions}, name=i
								[{ActivationFunctions}, name=c]
								[{CustomFunction}]
						  	]
							[{INeural\\Network}
								[{BasicNeuralNetwork}]
					...
					\node[anchor=south,align=left] 
					  at ([yshift=1cm]i|-c) {rozhraní};
					\node[anchor=south,align=left] 
					  at ([yshift=1cm]c|-c) {třídy, enumerates};
				\end{forest}
			...
			\section{core}
				...
				\subsection{IActivationFunctions}
					Rozhraní (anglicky \gls{interface}), které zahrnuje \verb!ActivationFunction! a \verb!CustomFunction!. Jeho instance se používají jako aktivační funkce. Funkce lze zavolat s~parametrem typu \verb!Double!, což nám dá hodnotu funkce v~tomto bodě, popřípadě lze obdobně zavolat jejich dvě metody \verb!xD! a~\verb!yD! udávající v~pořadí hodnotu derivace v~bodě x a~v~bodě, kde je funkční hodnota rovna parametru.
			
				\subsection{ActivationFunctions}
					Enumerate (česky výčet) častých funkcí, jež se používají jako aktivační funkce v~neuronech. Některé jsou označeny jako překonané (anglicky deprecated), jelikož u~funkcí, které nejsou všude hladké, neexistuje všude derivace. Taktéž u~funkcí, jež nejsou prosté, nelze vždy určit derivaci podle funkční hodnoty.
					
					Implementovány jsou všechny funkce uvedené v~kapitole \ref{s:af}
					
				\subsection{CustomFunction}
					Poskytuje možnost implementovat si vlastní aktivační funkci, má stejné metody (zde jsou to vlastnosti typu \verb!() -> Unit!) jako ActivationFunctions.
			
				\subsection{INeuralNetwork}
					Rozhraní, které implementuje základní funkce neuronových sítí, které mají jako vstup i~výstup vektor \verb!Double!. Obsahuje funkce:
					\begin{itemize}
				...
				\subsection{ConvolutionalNetwork}
					Tato třída rozhraní INeuralNetwork implementuje konvoluční neuronové sítě. Její konstruktor přijímá dva parametry typu \verb!BasicNeuralNetwork!, první je filtr, druhá je samotná neuronová síť. Dalším parametrem je logická hodnota, zda se má i~filtr učit (to se ale téměř nepoužívá, takže je tato hodnota při neuvedení nastavena na \verb!false!).
					
					Companion objekt této třídy navíc obsahuje příklad takového filtru (jednoduchý filtr detekující hrany viz obrázek \ref{fig:filter})
					\begin{figure}
						TODO(FILTER)
		...
		\chapter{Používání knihovny}
			\section{Trénování sítě}
				...
				Nejprve musíme neuronovou síť natrénovat a uložit. Trénování neuronové sítě probíhá za pomoci funkce \verb!train!. Té musíme poskytovat tréninkové vstupy s odpovídajícími výstupy, což můžeme udělat tak, že funkci \verb!train! budeme volat z cyklu, který bude tato data postupně načítat. Dalším způsobem je předat rovnou celý \verb!Array! vstupů a výstupů, to ale často znamená načíst miliony objektů třídy \verb!Double!, proto to může výrazně zpomalit učení. Poslední možností (pokud máme data ve formátu MNIST) je využít třídy \verb!TrainingData!, které poskytneme soubory s daty a ona vytvoří příslušné objekty typu \verb!Double! až ve chvíli, kdy dojde na danou dvojici vstup -- výstup.
	...
	\chapter*{Závěr}
		...
		Cílem mé práce bylo implementovat neuronovou síť, což se mi podařilo dokonce do takové míry, že v programu, kde zabírá pár řádků, je schopna rozeznávat číslice (ukázka je na stránkách \url{moznabude.cz}, nebo na přiloženém USB). Největším přínosem je asi třída \verb!BasicNeuralNetwork!, která implementuje velkou část matematiky obtížnou na rozmyšlení a stojící za téměř všemi neuronovými sítěmi, o niž se programátor v Kotlinu díky mojí knihovně už nemusí starat.
		
		Zároveň jsem si díky rozdělení do balíčků a využití možností objektově orientovaného programování připravil dobrý podklad pro rozšiřování knihovny. Dále bych mohl pokračovat například implementováním lepšího ukládání do souboru (ukládání typu \verb!Double! jako textového řetězce není moc efektivní), implementování některých genetických algoritmů, či naprogramování konvoluční sítě tak, aby filtry mohly pracovat $n$ rozměrně.
    ...
    \begin{prilohy}
    	...
    	\item{Porovnání grafů aktivačních funkcí} (následující stránky)
    	\item{Zdrojový kód knihovny} (následující stránky)
    \end{prilohy}  
    
	\addcontentsline{toc}{section}{Porovnání grafů aktivačních funkcí}	
	\begin{figure}
				    \centering
				    \begin{subfigure}[b]{0.3\textwidth}
