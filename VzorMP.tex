...
Dobrý popis programu v maturitní práci neobsahuje jen výpis kódu, ale také schéma,
které čtenáři rychle ukáže \emph{co} program dělá a \emph{jak} jsou jeho části propojeny.
Schéma je vhodné navrhnout \emph{před} psaním kódu – nutí vás promyslet strukturu dříve,
než se ponoříte do detailů implementace.

\textbf{Jak sestavit blokové schéma:}
\begin{itemize}
	\item každý větší celek (soubor, modul, třída, funkce) = jeden blok
	\item šipky = tok dat nebo volání funkce; na šipku napište \emph{co} teče nebo \emph{co} se volá
	\item datové soubory a databáze odlište od modulů (jiný tvar nebo barva)
	\item schéma nemusí být úplné – zachyťte hlavní vazby, ne každý řádek kódu
\end{itemize}

\textbf{Blokové schéma} projektu (viz také Příloha~B):
...
Pro \textbf{sekvenční diagramy} (kdo koho volá a v jakém pořadí) a složitější
schémata doporučujeme nástroj \href{https://www.diagrams.net}{draw.io}
nebo textový generátor \href{https://plantuml.com}{PlantUML}.
Výsledek exportujte jako PDF nebo PNG a vložte příkazem \verb|\includegraphics|.
